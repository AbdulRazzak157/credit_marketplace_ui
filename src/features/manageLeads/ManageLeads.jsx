...
const ManageLeads = () => {
    ...
    const getManageLeadCards = async () => {
        try {
            const token = await getAccessToken();
            const response = await fetch(`${API_URL.leadManagement.getManageLeadCards}`, {
                method: "GET",
                headers: {
            ...
            console.log("Result from get lead cards API: ", result.response);

            const data = {
                totalLeads: result?.response?.totalLeads || 0,
                new: result?.response.bew || 0,
                inReview: result?.response?.inReview || 0,
                approved: result?.response?.approved || 0,
                rejected: result?.response?.rejected || 0,
                disbursed: result?.response?.disbursed || 0,
                amount: result?.response?.amount || 0
            }

            return data;

        } catch (error) {
            console.log("Error in fetch lead cards api : ", error?.message);
        }
    }

    const { data: manageLeadCards, isLoading } = useQuery({
        queryKey: ["fetchManageLeadCards"],
        queryFn: getManageLeadCards
    });
    ...
    const { data: manageLeadData, isLoading: isLoading2 } = useQuery({
        queryKey: ["fetchManageLeadList",
            currentPage,
            rowsPerPage,
            debounceGeoSearch,
    ...
    return (
        <div className='space-y-4'>
            <h2 className='text-2xl text-(--primary) font-semibold'>Manage Leads</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-4 place-items-end">
                    <div className="inline-flex relative items-center w-full">
                        <Flatpickr
                            ...
                            className="border border-[#d1d5db] rounded-md outline-none px-4 py-2.5 pr-10 text-sm w-full"
                            options={{
                                mode: "single",
                                dateFormat: "d-m-Y",
                                disableMobile: true,
                    ...
                    <div className="inline-flex relative items-center w-full">
                        <Flatpickr
                            ...
                            className="border border-[#d1d5db] rounded-md outline-none px-4 py-2.5 text-sm w-full"
                            options={{
                                mode: "single",
                                dateFormat: "d-m-Y",
                                maxDate: new Date(),
            ...
            <div className='bg-white rounded-md px-2 md:px-4 py-4 md:py-6 space-y-4'>
                ...
                <DataTableBase
                    ...
                    paginationTotalRows={manageLeadData?.totalLeads}
