...
const AddLenderSupport = () => {
  ...
  const addNewLenderSupport = async (data) => {
    console.log("data: ", data);
    try {
      const token = await getAccessToken();
      const response = await fetch(`${API_URL.lenderSupport.addLenderSupport(id)}`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          contactPreference: ["email", "mobile"]
        })
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData?.response?.message || "Failed to add Lender Support");
      }
      toast.success("Lender Support Added Successfully");
      reset();
  ...
  return (
    <div>
      <div>
        <h2 className='text-lg text-(--primary) font-semibold'>Add New Contact</h2>
      </div>
      <form action="" onSubmit={handleSubmit(addNewLenderSupport)} className='bg-white grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-12 px-3 sm:px-8 xl:px-12 py-4 sm:py-8 rounded-md'>
        <div className='flex flex-col gap-1'>
          <label htmlFor="support-name" className='text-gray-600 text-sm sm:text-base'>Enter First Name*</label>
          <input
            id="supportName"
            type="text"
        ...
        <div className='flex flex-col gap-1'>
          ...
          <div className="flex items-center gap-2 justify-between border border-gray-300 px-4 py-1.5 rounded-md">
...
export default AddLenderSupport