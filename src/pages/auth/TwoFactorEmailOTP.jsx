...
const TwoFactorEmailOTP = () => {
    ...
    const { otpSession, userEmail, loginWithCustomToken } = useAuth();
    ...
    const requestOtpHandler = async () => {
        ...
        try {
            ...
            toast.success("Login Verification OTP Send");
        } catch (error) {
    ...
    const verifyLoginOtpHandler = async () => {
        ...
        try {
            ...
            toast.error(error?.message)
            console.log("Error in Verify Login OTP :", error?.message);
        }
...
export default TwoFactorEmailOTP